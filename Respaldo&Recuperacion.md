...
## Configuración de Storage VM + Volumen en NetApp

### Paso 1: Configurar Storage VM
1. Acceder a `192.168.150.10` en el navegador, iniciar sesión con `admin` y `netapp123`.
2. Ir a `Storage` → `Storage VMs` → `Add Storage VM` → Crear con nombre `svm0`.
3. Habilitar `NFS`, y en `Allow NFS Client Access`, añadir `192.168.150.20/24`(ip del ESXi) + `192.168.150.153` (ip del veeam).
4. Permitir `Read/Write Access & Superuser Access` y guardar.

### Paso 1.1: Crear Volumen (IP del NFS 192.168.150.12)
...
## Configuración de Virtual + Storage Infrastructure en Veeam

### Paso 1: Añadir Infraestructura de Virtualización
...
3. Seleccionar `NFS`
3. Tickear `Role: Block or file storage for VMware vSphere` con credenciales `admin` + `netapp123`.
4. Verificar que `svm0 → DS1 & svm0_root` estén listados.

## Configuración de Datastore en VMware ESXi + Creación de VM
### Paso 1: Configurar Datastore en VMware ESXi
1. Copiar el acceso NFS de `DS1`, `192.168.150.12:/DS1` desde el volumen de ESXi.
2. Ir a VMware ESXi (`192.168.150.20`) e iniciar sesión con `root` & `Password.01.`
3. Crear `New DataStore` → `Mount NFS datastore`.
4. Nombre: `DS1`, `NFS Server: 192.168.150.12`, `NFS Share: /DS1`, `NFS Version: NFS 3`.

### Crear Máquina Virtual en VMware ESXi
...
## Configuracion de Backup Jobs en Veeam

### Añadir VM a Veeam Backup Job
...
### Verificar Restauración en VMware ESXi
...
### Migrar a Producción o Desmontar VM Restaurada
